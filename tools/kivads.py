...
if platform == "android":
    ...
    RequestConfiguration = autoclass(
    ...
    class RewardedInterstitial:
        ...
        def load(self, UnitID):
            ...
            if not self.callback.loaded:
                ...
                _RewardedInterstitialAd.load(
                    context, UnitID, builder, self.callback)
            else:
                Logger.info(
                    "KivAds: Interstitial Ad already Loaded and Ready to Show")
        ...
        def show(self, immersive=False):
            ...
            if self.callback.loaded:
                if immersive:
                    self.callback.mRewardedInterstitialAd.setImmersiveMode(
                        True)
                ...
                self.callback.mRewardedInterstitialAd.show(
                    mActivity, self.reward_listener)
                self.callback.loaded = False
            else:
